...
#设置五折交叉验证
    ...
    # 对两个异质图的训练边进行标记
        ...
        feat_ml = torch.cat([g_ml.ndata['m_sim'], g_ml.ndata['l_sim']], dim=1)

        dl_edges = g0.filter_edges(lambda edges: edges.data['dl'])
        g_dl = g0.edge_subgraph(dl_edges, preserve_nodes=True)
        feat_dl = torch.cat([g_dl.ndata['d_sim'], g_dl.ndata['l_sim']], dim=1)

        if model_type == 'PNAGAT':
            model = PNAGAT(G0=g_train0,
                           meta_paths_list=['md', 'ml', 'dl'],
        ...
        for epoch in range(epochs):
            ...
            with torch.autograd.set_detect_anomaly(True):
                score_train = model(g_train0, g_train, src_train, dst_train)
            ...
            model.eval()
            with torch.no_grad():
                score_val = model(g0, g, src_test, dst_test)
            ...
            if (epoch + 1) % 10 == 0:
                print('Epoch:', epoch + 1, 'Train Loss: %.4f' % loss_train.item(),
                      ...
                      'Train AUC: %.4f' % train_auc,  'Time: %.2f' % (end - start))

        model.eval()
        with torch.no_grad():
            score_test = model(g0, g, src_test, dst_test)
        ...
        print('Fold: ', 'Test acc: %.4f' % acc_test, 'Test Pre: %.4f' % pre_test,
