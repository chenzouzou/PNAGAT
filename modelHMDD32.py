...
class PNAGAT(nn.Module):
    def __init__(self, G0, meta_paths_list, feature_attn_size, num_heads, num_diseases, num_mirnas, num_lncrnas,
        ...
        self.g_ml = g_ml
        self.convm = dgl.nn.PNAConv(501, 512,
        ...
        self.g_dl = g_dl
        self.convd = dgl.nn.PNAConv(496, 512,
                              # aggregators=['mean', 'max', 'sum', 'var', 'moment3', 'moment4'],
                              # aggregators=['mean', 'max', 'sum', 'var', 'std', 'moment3', 'moment4'],
                              aggregators=['mean', 'moment4'],
                              scalers=['identity'],
        ...
        self.d_fc0 = nn.Linear(432, 512)
        self.m_fc0 = nn.Linear(437, 512)
        # self.d_fc = nn.Linear(512, out_dim)
        # self.m_fc = nn.Linear(512, out_dim)
    ...
    def forward(self, G0, G, diseases, mirnas):   # 这里试一试只用G训练集，不用到G0所有的数据
        ...
        mirna1 = h_agg1[432:869]
        mirna2 = h_agg2[432:869]
        ...
        G_m_sim = G.ndata['m_sim'][self.num_diseases:869]
